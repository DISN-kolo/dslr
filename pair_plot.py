#!venv/bin/python

import pandas as pd
import numpy as np
import sys
import matplotlib.pyplot as plt

from utils.csv_reading import read_or_raise
import utils.additional_exceptions
from utils.additional_utils import (
    exit_with_print,
    exit_by_exception,
)
from utils.stat_helpers import *
from histogram import draw_hist
from scatter_plot import draw_scatter_plot

def get_nans_cached(cache, data, field):
    if (field not in cache):
        cache[field] = set(just_get_nans(data[field]))
    return cache[field]

def draw_pairplots_of(
        df,
        fields_to_draw,
        houses,
        test_df=None,
        predictions_df=None):
    if (df is None):
        raise DfNoneError
    all_houses = df["Hogwarts House"]
    all_nans = {}
    test_houses = None
    test_all_nans = {}
    if (test_df is not None and predictions_df is not None):
        test_houses = predictions_df.set_index('Index')['Hogwarts House']

    def draw_test_overlay(plot, field_x, field_y):
        if (test_houses is None):
            return
        test_nanned_stuff_x = get_nans_cached(test_all_nans, test_df, field_x)
        test_nanned_stuff_y = get_nans_cached(test_all_nans, test_df, field_y)
        current_pair = test_df[[field_x, field_y]].drop(
            index=test_nanned_stuff_x.union(test_nanned_stuff_y)
        )
        final_grouping = current_pair.groupby(test_houses)
        draw_scatter_plot(
            plot,
            houses,
            final_grouping,
            marker='x',
            show_label=False
        )

    fig, plots = plt.subplots(len(fields_to_draw), len(fields_to_draw))

    index_y = 0
    for field_y in fields_to_draw:
        print(f"   entered y: {index_y:3d}: {field_y:20}")
        nanned_stuff_y = get_nans_cached(all_nans, df, field_y)

        index_x = 0
        for field_x in fields_to_draw:
            print(f" entered x: {index_x:3d}: {field_x:20}")
            if (field_x == field_y):
                local_houses = all_houses.copy().drop(index=nanned_stuff_y)

                col_data = df[field_y].drop(index=nanned_stuff_y)
                final_grouping = col_data.groupby(local_houses)

                print("histogram draw called")
                draw_hist(
                    plots[index_y, index_x],
                    houses,
                    final_grouping
                )
                index_x += 1
                continue

            nanned_stuff_x = get_nans_cached(all_nans, df, field_x)
            nanned_stuff_xy = nanned_stuff_x.union(nanned_stuff_y)

            current_pair = df[[field_x, field_y]].drop(index=nanned_stuff_xy)
            local_houses = all_houses.copy().drop(index=nanned_stuff_xy)

            final_grouping = current_pair.groupby(local_houses)
            print("scatter draw called")
            draw_scatter_plot(
                plots[index_y, index_x],
                houses,
                final_grouping
            )
            draw_test_overlay(plots[index_y, index_x], field_x, field_y)
            index_x += 1
        index_y += 1

    for index, field in enumerate(fields_to_draw):
        plots[0, index].set_title(field)
        plots[index, 0].set_ylabel(field)

    for single_plot in plots.flat:
        single_plot.set_box_aspect(1)

    handles, labels = plots[0, 0].get_legend_handles_labels()
    fig.legend(handles, labels, loc='outside lower right')
    fig.subplots_adjust(top=0.98, bottom=0.02)
    plt.show()


if __name__=="__main__":
    if (len(sys.argv) != 2 and len(sys.argv) != 4):
        exit_with_print(
            1,
            f"Usage: {sys.argv[0]} <path/to/csv> "\
            "[<path/to/test.csv> <path/to/predictions.original_index>]"
        )
    try:
        df = read_or_raise(sys.argv[1])
        test_df = None
        predictions_df = None
        if (len(sys.argv) == 4):
            test_df = read_or_raise(sys.argv[2])
            predictions_df = read_or_raise(sys.argv[3])
        draw_pairplots_of(df, [
                "Astronomy",
                "Divination",
                "History of Magic",
                "Muggle Studies",
                "Charms",
                "Ancient Runes",
            ],
            [
                "Ravenclaw",
                "Slytherin",
                "Gryffindor",
                "Hufflepuff",
            ],
            test_df,
            predictions_df)
    except Exception as exc:
        exit_by_exception(exc, sys.argv[1])
